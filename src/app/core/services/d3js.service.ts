...
export class D3jsService {
  ...
  initChart(domElement: any, graph: any) {
    ...
    const strength: number = forceStrength > -35 ? -35 : forceStrength;
  ...
  initRdfChart(domElement: any, graph: any) {
    this.graph = graph;
    const colorScale = d3.scaleOrdinal(d3.schemeCategory10);

    this.renderedD3SVG = d3
      .select(domElement)
      .append('svg')
      .attr('id', 'graph')
    ...
    this.simulation = d3
      ...
      .force(
        ...
        d3
          .forceLink()
          .id(({ id }: any) => id)
          .distance(({ value }: any) => (value === 10 ? 100 : 50)),
      )
      .force('charge', d3.forceManyBody().strength(strength))
      .force('center', d3.forceCenter(this.width / 2, this.height / 2));
    ...
    this.renderedD3SVG
      .append('svg:defs')
      .selectAll('marker')
      .data([{ type: 'predicate' }, { type: 'argument' }]) // TODO Faire plutot [{ type: 'subject' }, { type: 'predicate' }, { type: 'object' }]
      .enter()
      .append('svg:marker')
      .attr('fill', ({ type }: any) => this.colorTypeRdf(type))
      .attr('id', ({ type }: any) => type)
      .attr('viewBox', '0 -5 10 10')
      .attr('refX', 19)
      .attr('refY', 0)
      ...
      .append('svg:path')
      .attr('d', 'M0,-5L10,0L0,5')
      .style('fill', ({ type }: any) => this.colorTypeRdf(type));

    // build the arrow for arguments.
    this.renderedD3SVG
      .append('svg:defs')
      .selectAll('marker')
      .data([{ type: 'predicate' }, { type: 'argument' }]) // Different link/path types can be defined here
      .enter()
      .append('svg:marker')
      .attr('fill', ({ type }: any) => this.colorTypeRdf(type))
      .attr('id', ({ type }: any) => type)
    ...
    this.links = this.renderedD3SVG
      ...
      .append('line')
      .attr('idPl', ({ source }: any) => source)
      .attr('stroke', ({ typeT }: any) => this.colorTypeRdf(typeT))
      .attr('stroke-dasharray', ({ dashed }: any) => (dashed ? 3 : 0))
      .attr('stroke-width', ({ value }: any) => Math.sqrt(value))
      .attr('marker-end', ({ typeT, target }: any) => {
        if (!this.isAPredicate(target, this.graph.nodes, [{ type: 'predicate' }, { type: 'argument' }])) {
          // is not predicate
          return 'url(#' + (typeT ? typeT.toLowerCase() : 'none') + ')';
        } else {
          // is predicate
          return 'url(#' + typeT.toLowerCase() + '2)';
        }
    ...
    this.nodes
      .append('circle')
      .attr('r', ({ type }: any) => this.sizeType(type))
      .attr('fill', ({ type }: any) => this.colorTypeRdf(type, true))
      .attr('stroke-dasharray', ({ dashed }: any) => (dashed ? 2 : 0))
      .call(
        d3
          .drag()
    ...
    this.nodes
      .append('text')
      .text(({ text }: any) => text)
      .style('fill', ({ type }: any) => this.colorTypeRdf(type, true))
      .attr('frame', ({ frame }: any) => (frame ? frame : null))
      .attr('wsd', ({ wsd }: any) => (wsd ? wsd : null))
      .attr('entityType', ({ entityType }: any) => entityType)
      .attr('x', ({ type }: any) => 1 + this.sizeType(type))
  ...
  colorTypeRdf(type: string, isAttribute = false) {
    let typeLowercase = type?.toLowerCase();
    if (isAttribute) {
      if (type.includes('predicate')) {
        return '#d10c02';
